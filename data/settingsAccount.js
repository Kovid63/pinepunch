import { Path, Svg } from "react-native-svg";

export const settingsAccount = [
    // {
    //     name: 'Account',
    //     icon: <Svg viewBox="0 0 24 25" xmlns="http://www.w3.org/2000/svg">
    //         <Path
    //             d="M19.84 18.193c0 3.296-4.52 3.677-7.92 3.677h-.242C9.512 21.865 4 21.728 4 18.173c0-3.229 4.338-3.66 7.711-3.676h.453c2.166.005 7.676.141 7.676 3.696Zm-7.92-2.197c-4.26 0-6.42.732-6.42 2.177 0 1.458 2.16 2.197 6.42 2.197s6.42-.732 6.42-2.177c0-1.458-2.16-2.197-6.42-2.197ZM11.92 2a5.315 5.315 0 0 1 5.31 5.31 5.314 5.314 0 0 1-5.31 5.309h-.031a5.3 5.3 0 0 1-5.28-5.312A5.316 5.316 0 0 1 11.922 2Zm0 1.428A3.887 3.887 0 0 0 8.039 7.31a3.873 3.873 0 0 0 3.854 3.882l.029.714v-.714A3.886 3.886 0 0 0 15.8 7.31a3.886 3.886 0 0 0-3.88-3.882Z"
    //             fill="#000000"
    //             fillRule="evenodd"
    //         />
    //     </Svg>
    // },
    // {
    //     name: 'Address',
    //     icon: <Svg viewBox="0 0 24 25" xmlns="http://www.w3.org/2000/svg">
    //         <Path
    //             d="M13.717 15.291a2.18 2.18 0 0 1 2.184 2.17v3.075c0 .257.206.463.47.47h1.906c1.502 0 2.723-1.207 2.723-2.689V9.593c-.007-.51-.25-.99-.667-1.309L13.74 3.026a2.416 2.416 0 0 0-3.011.002L4.18 8.282A1.675 1.675 0 0 0 3.5 9.61v8.707c0 1.482 1.22 2.688 2.723 2.688h1.924a.486.486 0 0 0 .49-.479c0-.058.008-.116.02-.17V17.46c0-1.189.974-2.16 2.169-2.169h2.89Zm4.56 7.214h-1.924c-1.102-.026-1.952-.89-1.952-1.969V17.46a.677.677 0 0 0-.684-.669H10.83a.677.677 0 0 0-.674.67v3.065c0 .075-.01.147-.031.215a1.99 1.99 0 0 1-1.98 1.764H6.224C3.893 22.505 2 20.626 2 18.317V9.603A3.154 3.154 0 0 1 3.259 7.1l6.535-5.245a3.912 3.912 0 0 1 4.88-.002l6.582 5.25a3.158 3.158 0 0 1 1.244 2.48v8.734c0 2.31-1.894 4.188-4.223 4.188Z"
    //             fill="#000000"
    //             fillRule="evenodd"
    //         />
    //     </Svg>
    // },
    {
        name: 'Notification',
        icon: <Svg viewBox="0 0 24 25" xmlns="http://www.w3.org/2000/svg">
            <Path
                d="M10.324 20.106c.518.577 1.183.894 1.873.894h.001c.693 0 1.361-.317 1.88-.895a.75.75 0 0 1 1.115 1.004c-.808.897-1.87 1.391-2.995 1.391h-.002c-1.12-.001-2.182-.495-2.987-1.392a.749.749 0 1 1 1.115-1.002ZM12.247 1c4.445 0 7.431 3.462 7.431 6.695 0 1.663.423 2.368.872 3.116.444.738.947 1.576.947 3.16-.349 4.047-4.574 4.377-9.25 4.377S3.345 18.018 3 14.035c-.003-1.648.5-2.486.944-3.224l.157-.264c.386-.663.715-1.385.715-2.852C4.816 4.462 7.802 1 12.247 1Zm0 1.5c-3.495 0-5.93 2.738-5.93 5.195 0 2.079-.578 3.04-1.088 3.888-.409.681-.732 1.219-.732 2.388.167 1.886 1.412 2.877 7.75 2.877 6.303 0 7.587-1.035 7.753-2.942-.003-1.104-.326-1.642-.735-2.323-.51-.848-1.087-1.809-1.087-3.888 0-2.457-2.436-5.195-5.93-5.195Z"
                fill="#000000"
                fillRule="evenodd"
            />
        </Svg>
    },
    {
        name: 'Payment Method',
        icon: <Svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            <Path
                d="M16.642 3a6.004 6.004 0 0 1 5.997 5.998v7.177a6.004 6.004 0 0 1-5.997 5.998H7.998A6.005 6.005 0 0 1 2 16.175V8.998A6.005 6.005 0 0 1 7.998 3h8.644Zm0 1.5H7.998A4.503 4.503 0 0 0 3.5 8.998v7.177a4.503 4.503 0 0 0 4.498 4.498h8.644a4.503 4.503 0 0 0 4.497-4.498v-.28H17.84a3.446 3.446 0 0 1-3.442-3.438 3.447 3.447 0 0 1 3.442-3.443l3.298-.001v-.015A4.503 4.503 0 0 0 16.642 4.5Zm4.497 6.013H17.84a1.945 1.945 0 0 0-1.942 1.943c0 1.069.872 1.94 1.942 1.94l3.298-.001v-3.882Zm-2.84 1.13a.75.75 0 0 1 0 1.5h-.313a.75.75 0 0 1 0-1.5h.312Zm-5.614-4.105a.75.75 0 0 1 0 1.5h-5.4a.75.75 0 0 1 0-1.5h5.4Z"
                fill="#000"
                fillRule="evenodd"
            />
        </Svg>
    },
    {
        name: 'Privacy',
        icon: <Svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 256">
        <Path fill={'#000000'} d="M128 24a104 104 0 1 0 104 104A104.118 104.118 0 0 0 128 24Zm0 192a88 88 0 1 1 88-88 88.1 88.1 0 0 1-88 88Zm-8-80V80a8 8 0 1 1 16 0v56a8 8 0 1 1-16 0Zm20 36a12 12 0 1 1-12-12 12.013 12.013 0 0 1 12 12Z" />
      </Svg>
    },
    {
        name: 'Security',
        icon: <Svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            <Path
                d="M16.334 2C19.724 2 22 4.378 22 7.916v8.168C22 19.622 19.723 22 16.332 22H7.664C4.276 22 2 19.622 2 16.084V7.916C2 4.378 4.276 2 7.664 2h8.67Zm0 1.5h-8.67C5.135 3.5 3.5 5.233 3.5 7.916v8.168c0 2.683 1.635 4.416 4.164 4.416h8.668c2.532 0 4.168-1.733 4.168-4.416V7.916c0-2.683-1.636-4.416-4.166-4.416ZM8.837 9.398c1.175.001 2.169.782 2.491 1.85l5.682.002a.75.75 0 0 1 .75.75v1.852a.75.75 0 0 1-1.5 0V12.75H14.93v1.103a.75.75 0 0 1-1.5 0V12.75l-2.102.001a2.607 2.607 0 0 1-2.492 1.852A2.605 2.605 0 0 1 6.235 12a2.605 2.605 0 0 1 2.602-2.602Zm.002 1.5a1.103 1.103 0 0 0-.002 2.204c.607 0 1.102-.494 1.102-1.102 0-.607-.494-1.1-1.1-1.102Z"
                fill="#000"
                fillRule="evenodd"
            />
        </Svg>
    },
]