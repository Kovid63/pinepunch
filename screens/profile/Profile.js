...
const Profile = ({ navigation }) => {
  ...
  function saveBtnHandler() {
    let details = {};
    merchantData.name && (details.name = merchantData.name);
    merchantData.address && (details.address = merchantData.address);
  ...
  return (
    <>
      ...
      {isModalOpen && <Pressable onPress={() => setIsModalOpen(false)} style={{ backgroundColor: 'rgba(0, 0, 0, 0.3)', position: 'absolute', height: '100%', width: '100%', justifyContent: 'center', alignItems: 'center' }}>
        <View style={{ backgroundColor: '#F8F8F8', width: 325, borderRadius: 25, justifyContent: 'space-evenly', paddingHorizontal: '5%', paddingVertical: 10 }}>
          <View style={{ flexDirection: 'row' }}>
            <Text style={[styles.infoTitle, { marginTop: 10 }]}>{'Company Name'}</Text>
          </View>
          <TextInput value={merchantData.name} style={[styles.infoText, { backgroundColor: '#fff', width: '100%' }]} onChangeText={(name) => { setMerchantData({ ...merchantData, name: name }) }} />
          <Text style={[styles.infoTitle, { marginTop: 10 }]}>{'Location'}</Text>

          <TextInput value={merchantData.address} multiline style={[styles.infoText, { backgroundColor: '#fff', width: '100%', paddingVertical: 2 }]} onChangeText={(address) => { setMerchantData({ ...merchantData, address: address }) }} />
          <View style={{ backgroundColor: '#F8F8F8' }}>
          ...
          <TouchableOpacity onPress={() => saveBtnHandler()} style={{ paddingVertical: 10, paddingHorizontal: '8%', backgroundColor: colors.primary[0], borderRadius: 16, alignItems: 'center', justifyContent: 'center', marginTop: 20, alignSelf: 'center' }}>
            <Text style={{ fontFamily: 'Poppins', color: '#FFFFFF', fontSize: 13 }}>{'Save'}</Text>
          </TouchableOpacity>
        </View>
      </Pressable>}
    </>
